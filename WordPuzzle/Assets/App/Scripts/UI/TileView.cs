...
namespace WordPuzzle.UI
{
    public class TileView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            // If not consumed by a drop, return to original parent (or Rack will handle it)
            // We'll let the event consumer handle parenting if drop was successful
            // Defaults to resetting if no one claimed it
            
            // Check if dropped successfully? 
            // Better pattern: DropHandler reports success.
            // For now, let's just make it float back if not reparented.
        }
